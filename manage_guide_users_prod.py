...
# -*- coding: utf-8 -*-
"""
Скрипт для управления пользователями гайдов в ПРОДАКШЕНЕ
...
if not mongo_uri:
    ...
    print("Добавьте в .env: MONGO_URI=mongodb://localhost:27018/")
    sys.exit(1)

if not mongo_db_name:
    print("❌ ОШИБКА: MONGO_DB_NAME не найден в .env файле")
    print("Добавьте в .env: MONGO_DB_NAME=stylist_ai_prod")
    sys.exit(1)

print(f"✅ MONGO_URI: {mongo_uri}")
print(f"✅ MONGO_DB_NAME: {mongo_db_name}")

# Теперь импортируем user_auth
from user_auth import user_auth

def print_banner():
    """Выводит красивый баннер"""
    print("=" * 60)
    print("🎨 УПРАВЛЕНИЕ ПОЛЬЗОВАТЕЛЯМИ ГАЙДОВ (ПРОДАКШЕН)")
    print("=" * 60)
    print()
...
def main():
    ...
    print(f"🗄️ База данных: {mongo_db_name}")
