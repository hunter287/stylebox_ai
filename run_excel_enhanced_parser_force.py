...
ПРИНУДИТЕЛЬНАЯ ВЕРСИЯ - игнорирует проверку на уже обработанные товары
Использование: python run_excel_enhanced_parser_force.py [start_row]
Пример: python run_excel_enhanced_parser_force.py 111  # начать с 111-й строки
"""
...
async def process_excel_products_force(start_row=1):
    """Обрабатывает товары из Excel файла и находит визуально похожие (ПРИНУДИТЕЛЬНО)"""
    parser = EnhancedSimilarParser()
    
    try:
        print("🚀 Запуск ПРИНУДИТЕЛЬНОГО парсинга товаров из Excel файла...")
        print(f"📍 Начинаем с строки Excel файла: {start_row}")
        print("⚠️ ВНИМАНИЕ: Проверка на уже обработанные товары ОТКЛЮЧЕНА!")
        ...
        print(f"\n🚀 Начинаем ПРИНУДИТЕЛЬНУЮ обработку {total_remaining} товаров из Excel файла...")
        ...
        for i, (index, row) in enumerate(remaining_products.iterrows()):
            ...
            try:
                ...
                # ПРИНУДИТЕЛЬНО обрабатываем товар (игнорируем проверку на уже обработанные)
                print(f"🔄 ПРИНУДИТЕЛЬНО обрабатываем товар (игнорируем проверку на уже обработанные)...")
        ...
        print(f"\n{'='*60}")
        print(f"📊 ИТОГОВЫЕ РЕЗУЛЬТАТЫ (ПРИНУДИТЕЛЬНЫЙ ПАРСИНГ)")
        print(f"{'='*60}")
        ...
        print("\n🎉 ПРИНУДИТЕЛЬНАЯ обработка всех товаров завершена успешно!")
        return True
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='ПРИНУДИТЕЛЬНЫЙ парсинг товаров из Excel файла с поиском похожих товаров',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Примеры использования:
  python run_excel_enhanced_parser_force.py          # начать с первой строки
  python run_excel_enhanced_parser_force.py 111      # начать с 111-й строки
  python run_excel_enhanced_parser_force.py 250      # начать с последней строки
  
⚠️ ВНИМАНИЕ: Этот скрипт игнорирует проверку на уже обработанные товары!
        """
    )
...
async def main():
    ...
    print(f"🚀 Запуск ПРИНУДИТЕЛЬНОГО парсинга с строки {args.start_row}")
    
    success = await process_excel_products_force(args.start_row)
    
    if success:
        print("\n✅ ПРИНУДИТЕЛЬНЫЙ парсинг завершен успешно!")
        print("🚀 Можно запускать обработку всех товаров")
    else:
        print("\n❌ ПРИНУДИТЕЛЬНЫЙ парсинг завершен с ошибками")
